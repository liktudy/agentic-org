...
async function listTemplates() {
  ...
  const descs = files.map(f => {
    ...
    return `  - ${path.basename(f, ".json")}: ${cfg.description || "No description"}`;
  });
...
module.exports = {
  ...
  toPptx: (args) => toPptx.toPptx(args.sourcePath, getTemplateConfig(args.template), resolveOutputDir(args.sourcePath), args.outputName),
  toDocx: (args) => toDocx.toDocx(args.sourcePath, getTemplateConfig(args.template), resolveOutputDir(args.sourcePath), args.outputName),
